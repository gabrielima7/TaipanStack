...
## 2026-07-21 - [Fix] Type Check and Bounds Check Validations Before math.isfinite()
**Vulnerability:** Found that multiple watchdog classes (`ConfigWatcher`, `HealthPinger`, `ResourceWatcher`, `BaseWatcher`), `timeout` decorators, and configuration models (`CircuitBreakerConfig`, `RetryConfig`) called `math.isfinite()` or evaluated numerical bounds (like `< 0`) without explicitly verifying the type with `isinstance(val, (int, float))` first. Additionally, some numerical configurations lacked proper `val < 0` domain bounds checks.
**Learning:** Calling `math.isfinite()` with a string, list, or dictionary parameter causes Python to raise an unhandled `TypeError` instead of a `ValueError` or a boolean validation failure. In resilience classes and watchdogs, `math.isfinite` and bounds checks (e.g. `<= 0`) must be strictly preceded by an explicit Look-Before-You-Leap (LBYL) type check to guarantee that malicious or erroneous configurations fail safely and predictably, avoiding unhandled `TypeError` panics from deep within the stack.
**Prevention:** Always use `isinstance(val, (int, float))` before calling `math.isfinite()` or evaluating numerical bounds on parameters, particularly in configuration initializers and decorators. Ensure `val < 0` limits are explicitly enforced alongside finiteness.
## 2026-07-21 - [Fix] Type Check and Bounds Check Validations Before math.isfinite()
**Vulnerability:** Found that multiple watchdog classes (`ConfigWatcher`, `HealthPinger`, `ResourceWatcher`, `BaseWatcher`), `timeout` decorators, and configuration models (`CircuitBreakerConfig`, `RetryConfig`) called `math.isfinite()` or evaluated numerical bounds (like `< 0`) without explicitly verifying the type with `isinstance(val, (int, float))` first. Additionally, some numerical configurations lacked proper `val < 0` domain bounds checks.
**Learning:** Calling `math.isfinite()` with a string, list, or dictionary parameter causes Python to raise an unhandled `TypeError` instead of a `ValueError` or a boolean validation failure. In resilience classes and watchdogs, `math.isfinite` and bounds checks (e.g. `<= 0`) must be strictly preceded by an explicit Look-Before-You-Leap (LBYL) type check to guarantee that malicious or erroneous configurations fail safely and predictably, avoiding unhandled `TypeError` panics from deep within the stack.
**Prevention:** Always use `isinstance(val, (int, float))` before calling `math.isfinite()` or evaluating numerical bounds on parameters, particularly in configuration initializers and decorators. Ensure `val < 0` limits are explicitly enforced alongside finiteness.

## 2026-07-24 - [Fix] Add LBYL type check to RateLimiter _validate_finite
**Vulnerability:** The RateLimiter initialization logic in `src/taipanstack/utils/rate_limit.py` invoked `math.isfinite()` on user-supplied `max_calls` and `time_window` parameters without first asserting they were numeric types (int, float) via an `isinstance` type guard.
**Learning:** Passing a string (e.g. from an unvalidated API payload) to `math.isfinite` triggers a runtime `TypeError` instead of a logical `ValueError` (which rate limiter handlers typically anticipate). This bypassed LBYL logic, potentially causing unhandled exceptions or DoS scenarios in higher level routing components if exception handlers didn't catch `TypeError`.
**Prevention:** Always strictly execute `isinstance(val, (int, float))` type guards directly preceding bounds checks or calls to `math.isfinite` on mathematical thresholds.

## 2026-07-25 - [Fix] Type Check and Bounds Check Validations Before math.isfinite() in CircuitBreaker and Retry
**Vulnerability:** Found that `src/taipanstack/resilience/circuit_breaker.py` and `src/taipanstack/resilience/retry.py` called `math.isfinite()` without explicitly verifying the type with `isinstance(val, (int, float))` first in multiple places (`_calculate_elapsed_time`, `_get_valid_elapsed`, `_update_failure_metrics`, `_calculate_base_delay`, `_compute_jitter_amount`, `_apply_jitter`, `calculate_delay`).
**Learning:** Calling `math.isfinite()` with a string, list, or dictionary parameter causes Python to raise an unhandled `TypeError` instead of a `ValueError` or a boolean validation failure. `math.isfinite` and bounds checks (e.g. `< 0`) must be strictly preceded by an explicit Look-Before-You-Leap (LBYL) type check to guarantee that malicious or erroneous configurations fail safely and predictably, avoiding unhandled `TypeError` panics from deep within the stack.
**Prevention:** Always use `isinstance(val, (int, float))` before calling `math.isfinite()` or evaluating numerical bounds on parameters. Ensure formatting tools like `ruff` don't complain about line length when doing this.

## 2026-08-03 - [Fix] TypeError in secrets.compare_digest for non-ASCII strings
**Vulnerability:** Found that `encode_jwt` and `decode_jwt` in `src/taipanstack/security/jwt.py` called `secrets.compare_digest()` directly on untrusted input strings (`algorithm` and items in `algorithms`) to explicitly deny the "none" algorithm.
