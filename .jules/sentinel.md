...
## 2026-07-20 - [Fix] Missing `math.isfinite` check for `AdaptiveRetry` initialization
**Vulnerability:** The `AdaptiveRetry` class in `src/taipanstack/resilience/adaptive/adaptive_retry.py` accepted parameters (`min_delay` and `max_delay`) without checking for bounds or finiteness (`math.isfinite()`).
**Learning:** This is a recurring anti-pattern within TaipanStack's resilience classes, where numeric bounds validate logic without checking if the value itself is valid (e.g. `NaN`, `infinity`). Failing to validate values at API boundaries can lead to Denial of Service (DoS) attacks via infinite loop blocking or unexpected `ValueError`s from Python's low-level synchronization primitives.
**Prevention:** Ensure numerical constraints always employ `math.isfinite()` bounds checks (along with typical `>= 0` checks) on API initialization before the variables are ever utilized within business logic.
