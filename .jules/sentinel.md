...
## 2026-08-01 - [Fix] Missing isfinite Bounds Check for TTL in Cache Decorator
**Vulnerability:** The `cached` decorator in `src/taipanstack/utils/cache.py` did not explicitly validate the bounds and type of its `ttl` parameter. Passing negative numbers or non-finite values (like `float('inf')` or `float('NaN')`) could bypass intended cache expiration bounds, leading to potential DoS due to memory exhaustion or unpredictable caching behavior.
**Learning:** In modular architectures wrapping caching primitives, parameter bounds validation must happen early at the API boundary. Relying on downstream libraries or time constraints without explicit bound checks creates silent logic flaws.
**Prevention:** Always explicitly use `math.isfinite()` and explicit bounds checking (e.g. `< 0`) alongside type checks when handling raw numerical inputs for Time-To-Live (TTL) values.
