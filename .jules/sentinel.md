...
## 2026-03-29 - Arbitrary Module Import Vulnerability
**Vulnerability:** The `require_dependency` function passed a user-supplied string directly to `importlib.import_module()`. This is flagged by Semgrep as a blocking arbitrary code loading vulnerability, as malicious user input could be structured to perform actions like command execution on import.
**Learning:** Using `importlib.import_module` without sanitization allows any string to be processed. While sometimes used defensively for optional dependencies, failing to constrain the input means potentially calling arbitrary Python expressions if those inputs are ever dynamic.
**Prevention:** If an explicit whitelist cannot be used for dynamic loading, ensure that all parts of the module path strictly conform to Python identifier constraints using `str.isidentifier()`. This ensures that only valid module names (without trailing function calls, strings, or arbitrary shell expressions) can be passed to the loader.
