...
def validate_url(
    ...
    *,
    ...
    if len(url) > MAX_URL_LENGTH:
        msg = f"URL exceeds maximum length of {MAX_URL_LENGTH}"
        raise ValueError(msg)
    ...
    if require_tld:
        # Check for TLD using hostname which handles creds/ports safely
        domain = parsed.hostname or ""
        # IPv6 addresses (e.g., [::1]) are parsed cleanly to ::1 via hostname
