...
def _validate_write_path(path: Path, base_dir: Path | str | None) -> None:
    """Validate path for safe writing."""
    if base_dir is not None:
...
def _validate_copy_paths(src: Path, dst: Path, base_dir: Path | str | None) -> Path:
    """Validate source and destination paths for safe copying.
    ...
    if base_dir is not None:
        base = Path(base_dir)
        validated_src = guard_path_traversal(src, base)
        # For dst, validate parent if file doesn't exist
        if dst.exists():
            guard_path_traversal(dst, base)
        else:
...
def safe_copy(
    ...
    *,
    ...
    # Validate paths
    src = _validate_copy_paths(src, dst, base_dir)
