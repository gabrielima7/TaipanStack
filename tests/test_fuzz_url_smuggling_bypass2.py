...
def test_url_smuggling_bypass_validators(depth: int) -> None:
    ...
    with pytest.raises(ValueError, match="URL contains invalid characters|nested encoding limit"):
        validate_url(url)
...
def test_url_smuggling_bypass_guards(depth: int) -> None:
    ...
    assert "URL contains invalid characters" in str(result.err_value) or "nested encoding limit" in str(result.err_value)
