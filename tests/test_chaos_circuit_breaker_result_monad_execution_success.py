...
def test_chaos_circuit_breaker_result_monad_circuit_breaker_with_err_monad_execution_success():
...
@pytest.mark.asyncio
async def test_chaos_circuit_breaker_result_monad_async_circuit_breaker_with_err_monad_execution_success():
    @circuit_breaker(failure_threshold=2, failure_exceptions=(ValueError,))
...
def test_chaos_circuit_breaker_result_monad_circuit_breaker_with_err_monad_not_in_exceptions_execution_success():
    @circuit_breaker(failure_threshold=2, failure_exceptions=(ValueError,))
    def flaky_function(fail: bool) -> Result[str, Exception]:
        if fail:
...
@pytest.mark.asyncio
async def test_chaos_circuit_breaker_result_monad_async_circuit_breaker_with_err_monad_not_in_exceptions_execution_success():
    @circuit_breaker(failure_threshold=2, failure_exceptions=(ValueError,))
    async def async_flaky_function(fail: bool) -> Result[str, Exception]:
        if fail:
            return Err(KeyError("Not tracked"))
