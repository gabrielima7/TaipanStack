...
@pytest.mark.asyncio
async def test_chaos_resilience_timeout_exceptions_chaos_timeout_async_cancelled_error_expected():
...
@pytest.mark.asyncio
async def test_chaos_resilience_timeout_exceptions_chaos_timeout_async_overflow_error_expected():
    def side_effect(*args, **kwargs):
        # Prevent "coroutine was never awaited" warning
        args[0].close()
...
def test_chaos_resilience_timeout_exceptions_chaos_timeout_sync_overflow_error_expected():
    with patch(
