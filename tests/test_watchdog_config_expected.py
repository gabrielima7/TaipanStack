...
class TestParseEnv:
    ...
    def test_watchdog_config_basic_parsing_keyvalue_pairs(self) -> None:
        """Parse simple key=value pairs."""
        text = "HOST=localhost\nPORT=8080\n"
        result = _parse_env(text)
        assert result == {"HOST": "localhost", "PORT": "8080"}

    def test_watchdog_config_skips_comments_and_blanks_are_ignored(self) -> None:
        """Comments and empty lines are ignored."""
        text = "# comment\n\nKEY=value\n"
        result = _parse_env(text)
        assert result == {"KEY": "value"}

    def test_watchdog_config_strips_quotes_from_values(self) -> None:
    ...
    def test_watchdog_config_skips_lines_without_equals_are_skipped(self) -> None:
...
class TestParseJson:
    ...
    def test_watchdog_config_valid_json_json_object(self) -> None:
        """Parses valid JSON object."""
        result = _parse_json('{"host": "db", "port": 5432}')
        assert isinstance(result, Ok)
        assert result.ok_value == {"host": "db", "port": 5432}

    def test_watchdog_config_invalid_json_malformed_json(self) -> None:
        """Returns Err for malformed JSON."""
        result = _parse_json("{broken")
        assert isinstance(result, Err)

    def test_watchdog_config_non_dict_json_an_object(self) -> None:
...
class TestHashFile:
    ...
    def test_watchdog_config_hash_existing_file_existing_file(self, tmp_path: Path) -> None:
    ...
    def test_watchdog_config_hash_missing_file_missing_file(self, tmp_path: Path) -> None:
...
class TestLoadFileData:
    ...
    def test_watchdog_config_load_json_file_json_file(self, tmp_path: Path) -> None:
    ...
    def test_watchdog_config_load_env_file_env_file(self, tmp_path: Path) -> None:
    ...
    def test_watchdog_config_unsupported_extension_file_extensions(self, tmp_path: Path) -> None:
    ...
    def test_watchdog_config_missing_file_missing_file(self, tmp_path: Path) -> None:
        """Returns Err for missing file."""
...
class TestValidateConfig:
    ...
    def test_watchdog_config_valid_data_valid_data(self) -> None:
    ...
    def test_watchdog_config_invalid_data_fails_validation(self) -> None:
...
class TestConfigWatcher:
    ...
    @pytest.mark.asyncio
    async def test_watchdog_config_start_stop_lifecycle_and_stopped(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_watchdog_config_detects_file_change_file_changes(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_watchdog_config_invalid_config_calls_error_callback_bad_config(
        self, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_watchdog_config_missing_file_does_not_crash_handled_gracefully(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_watchdog_config_detect_changes_no_change_been_modified(self, tmp_path: Path) -> None:
    ...
    def test_watchdog_config_validate_and_apply_with_env_file_env_files(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_watchdog_config_validate_and_apply_missing_file_be_loaded(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_watchdog_config_validate_and_apply_invalid_without_error_callback_is_none(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_watchdog_config_validate_and_apply_valid_without_change_callback_is_none(
        self, tmp_path: Path
    ) -> None:
...
@pytest.mark.asyncio
async def test_watchdog_config_config_watcher_change_detection_error_coverage_error_logging() -> (
    None
):
