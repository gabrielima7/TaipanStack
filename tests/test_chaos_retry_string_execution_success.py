...
def test_chaos_retry_string_max_attempts_execution_success() -> None:
...
@pytest.mark.asyncio
async def test_chaos_retry_string_max_attempts_async_execution_success() -> None:
    @retry(max_attempts="3")  # type: ignore
...
def test_chaos_retry_string_max_attempts_failure_execution_success() -> None:
...
@pytest.mark.asyncio
async def test_chaos_retry_string_max_attempts_async_failure_execution_success() -> None:
    @retry(max_attempts="3", on=ValueError)  # type: ignore
    async def my_func() -> Ok[str]:
        raise ValueError("test")
