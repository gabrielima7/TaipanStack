...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_expected() -> (
    None
):
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "initial_delay", "string_mutation")
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails2_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "initial_delay", "string_mutation")
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails3_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails4_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "max_delay", "string_mutation")
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails5_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "max_delay", 1.0)
...
def test_chaos_retry_type_mutation_apply_jitter_mutation_delay2_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails7_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "exponential_base", 1)
...
def test_chaos_retry_type_mutation_calculate_delay_type_mutation_delay_all_fails8_expected() -> (
    None
):
    config = RetryConfig(initial_delay=1.0, max_attempts=3, max_delay=60.0)
    object.__setattr__(config, "exponential_base", 2.0)
...
def test_chaos_retry_type_mutation_retry_config_init_type_mutation_expected() -> (
    None
):
    config = RetryConfig(
...
def test_chaos_retry_on_exception_expected() -> None:
    from taipanstack.resilience.retry import RetryError, retry_on_exception

    @retry_on_exception((ValueError,), max_attempts=2)
    def test_func_standard_expected():
        raise ValueError("test")

    with pytest.raises(RetryError):
        test_func_standard_expected()


def test_chaos_retry_exit_should_retry_false_due_to_none_exc_val_expected() -> (
    None
):
...
def test_chaos_retry_exit_success_expected() -> None:
    r = Retrier()
    r.__exit__(None, None, None)


def test_chaos_retry_should_retry_type_error_for_issubclass_expected() -> None:
    """
...
@pytest.mark.asyncio
async def test_chaos_retry_decorator_type_error_for_isinstance_expected() -> (
    None
):
    """
