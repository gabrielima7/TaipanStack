...
malformed_payload_strategy = st.one_of(
    st.integers(), st.floats(), st.booleans(), st.none(), st.lists(st.text()), st.text()
)
...
class TestFuzzJWT:
    ...
    def test_fuzz_encode_jwt_malformed_payload_expected(
    ...
    @settings(max_examples=500, suppress_health_check=[HealthCheck.too_slow])
    def test_fuzz_decode_jwt_malformed_token_expected(
    ...
    @settings(max_examples=500, suppress_health_check=[HealthCheck.too_slow])
    def test_fuzz_encode_jwt_malformed_secret_expected(
        self, payload, secret, algorithm
    ) -> None:
    ...
    @settings(max_examples=500, suppress_health_check=[HealthCheck.too_slow])
    def test_fuzz_encode_jwt_malformed_algorithm_expected(
