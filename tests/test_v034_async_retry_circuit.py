...
class TestRetryAsyncSupport:
    ...
    async def test_async_success_no_retry_expected(self) -> None:
    ...
    async def test_async_retries_then_succeeds_expected(self) -> None:
    ...
    async def test_async_raises_retry_error_after_max_attempts_expected(self) -> None:
    ...
    async def test_async_uses_asyncio_sleep_not_time_sleep_expected(self) -> None:
        ...
        async def one_fail() -> str:
            ...
            return "ok"
        with patch("taipanstack.resilience.retry.asyncio.sleep", mock_asyncio_sleep), patch("taipanstack.resilience.retry.time.sleep", mock_time_sleep):
            result = await one_fail()
        assert result == "ok"
        mock_asyncio_sleep.assert_awaited_once()
        mock_time_sleep.assert_not_called()

    async def test_async_on_retry_callback_is_called_expected(self) -> None:
        """on_retry callback is invoked on each failed async attempt."""
        callback_calls: list[tuple[int, int, Exception, float]] = []

        @retry(max_attempts=3, initial_delay=0.0, jitter=False, on=(ValueError,), on_retry=lambda a, m, e, d: callback_calls.append((a, m, e, d)))
    ...
    async def test_async_structlog_warning_called_without_callback_expected(self) -> None:
        ...
        with patch("taipanstack.resilience.retry._HAS_STRUCTLOG", True), patch("taipanstack.resilience.retry._structlog_logger", mock_structlog_logger), patch("taipanstack.resilience.retry.asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_async_log_retries_false_skips_logging_expected(self) -> None:
        """log_retries=False suppresses stdlib log messages for async functions."""
        call_count = 0

        @retry(max_attempts=2, initial_delay=0.0, jitter=False, on=(ValueError,), log_retries=False)
    ...
    async def test_async_reraise_false_still_raises_retry_error(self) -> None:
        ...
        @retry(max_attempts=1, initial_delay=0.0, jitter=False, on=(ValueError,), reraise=False)
...
class TestCircuitBreakerAsyncSupport:
    ...
    async def test_async_success_keeps_closed_expected(self) -> None:
    ...
    async def test_async_failures_open_circuit_expected(self) -> None:
    ...
    async def test_async_open_circuit_raises_circuit_breaker_error(self) -> None:
        ...
        breaker = CircuitBreaker(failure_threshold=1, timeout=60.0, name="test_async_block")
    ...
    async def test_async_half_open_success_closes_circuit_expected(self) -> None:
        """Async success in HALF_OPEN transitions circuit to CLOSED."""
        breaker = CircuitBreaker(failure_threshold=1, success_threshold=1, timeout=0.05, name="test_async_half_open")
        call_count = 0
    ...
    async def test_async_half_open_failure_reopens_circuit_expected(self) -> None:
        """Async failure in HALF_OPEN transitions circuit back to OPEN."""
        breaker = CircuitBreaker(failure_threshold=1, timeout=0.05, name="test_async_reopen")
    ...
    async def test_circuit_breaker_decorator_async_expected(self) -> None:
    ...
    async def test_async_excluded_exceptions_dont_trip_expected(self) -> None:
        """Excluded exceptions don't count as failures for async functions."""
        breaker = CircuitBreaker(failure_threshold=2, excluded_exceptions=(ValueError,), name="test_async_excluded")
    ...
    async def test_async_on_state_change_callback_called_expected(self) -> None:
        """on_state_change callback fires on async-triggered state transitions."""
        transitions: list[tuple[CircuitState, CircuitState]] = []
        breaker = CircuitBreaker(failure_threshold=1, name="test_async_cb", on_state_change=lambda o, n: transitions.append((o, n)))
    ...
    async def test_async_structlog_called_on_state_change_no_callback_expected(self) -> None:
        ...
        with patch("taipanstack.resilience.circuit_breaker._HAS_STRUCTLOG", True), patch("taipanstack.resilience.circuit_breaker._structlog_logger", mock_structlog_logger):
            with pytest.raises(RuntimeError):
                await fail()
        assert mock_structlog_logger.warning.call_count >= 1
