...
def test_circuit_breaker_type_corruption_half_open_attempts() -> None:
    breaker = CircuitBreaker(name="test_half_open", failure_threshold=3, success_threshold=2)
...
def test_circuit_breaker_type_corruption_success_count() -> None:
    breaker = CircuitBreaker(name="test_success", failure_threshold=3, success_threshold=2)
    breaker._state.state = CircuitState.HALF_OPEN
    # Induce type mutation
...
def test_circuit_breaker_type_corruption_failure_count_in_update() -> None:
    breaker = CircuitBreaker(name="test_failure_update", failure_threshold=3, success_threshold=2)
...
def test_circuit_breaker_type_corruption_handle_failure_closed_direct() -> None:
    breaker = CircuitBreaker(name="test_failure_closed", failure_threshold=3, success_threshold=2)
    breaker._state.state = CircuitState.CLOSED
