...
class TestGuardSsrfTypeContract:
    ...
    def test_raises_err_for_int_expected(self) -> None:
        """Return Err when URL is an integer."""
        result = guard_ssrf(123)
        assert result.is_err()

    def test_raises_err_for_none_expected(self) -> None:
        """Return Err when URL is None."""
        result = guard_ssrf(None)
        assert result.is_err()

    def test_raises_err_for_bytes_expected(self) -> None:
        """Return Err when URL is bytes."""
        result = guard_ssrf(b"http://example.com")
        assert result.is_err()

    @patch("taipanstack.security.guards.urlsplit")
    def test_raises_value_error_from_urlparse_expected(self, mock_urlparse) -> None:
...
class TestGuardSsrfEmptyAndMalformed:
    ...
    def test_empty_url_returns_err_expected(self) -> None:
    ...
    def test_disallowed_scheme_ftp_returns_err_expected(self) -> None:
    ...
    def test_disallowed_scheme_file_returns_err_expected(self) -> None:
        """file:// scheme is rejected."""
        result = guard_ssrf("file:///etc/passwd")
        assert result.is_err()

    def test_no_hostname_returns_err_expected(self) -> None:
