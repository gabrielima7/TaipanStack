...
def test_fuzz_sanitize_filename_malformed_replacement_execution_success(
    replacement,
) -> None:
...
@settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
def test_fuzz_sanitize_filename_malformed_preserve_extension_execution_success(
    preserve_extension,
) -> None:
...
@settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
def test_fuzz_sanitize_filename_malformed_max_length_execution_success(
    max_length,
) -> None:
