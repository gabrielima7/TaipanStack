...
@pytest.mark.asyncio
async def test_concurrency_async_timeout_mutation_expected():
...
def test_concurrency_sync_timeout_mutation_expected():
    @limit_concurrency(max_tasks=1, timeout=1.0)
    def fast_op_sync():
        return "success"
...
def test_concurrency_timeout_type_mutation_expected():
    with pytest.raises(
...
def test_concurrency_max_tasks_type_mutation_expected():
