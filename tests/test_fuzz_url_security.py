...
@settings(suppress_health_check=[HealthCheck.large_base_example, HealthCheck.data_too_large], max_examples=200)
@given(st.text(min_size=MAX_URL_LENGTH + 1, max_size=MAX_URL_LENGTH + 1000))
def test_fuzz_url_massive_strings_dos_expected(url: str) -> None:
...
@given(st.text(alphabet=st.characters(blacklist_categories=("Cc",), whitelist_characters=["\x00"]), min_size=10, max_size=100).filter(lambda s: "\x00" in s))
def test_fuzz_url_null_bytes_expected(url: str) -> None:
    """Fuzz validate_url with strings containing null bytes."""
    with pytest.raises(ValueError, match="URL contains invalid characters"):
        validate_url(url)

@settings(suppress_health_check=[HealthCheck.filter_too_much])
@given(st.text(alphabet=st.characters(blacklist_characters=["\x00"], min_codepoint=8203, max_codepoint=8207), min_size=1, max_size=10))
def test_fuzz_url_unprintable_chars_expected(chars: str) -> None:
    """Fuzz validate_url with zero-width characters and unprintable unicode."""
    url = f"http://example.com/{chars}"
    with pytest.raises(ValueError, match="URL contains invalid characters"):
        validate_url(url)

def test_url_port_integer_conversion_dos_expected() -> None:
    """Explicitly test huge port sizes that crash URL parsers."""
    url = "http://example.com:" + "9" * 10000
    with pytest.raises(ValueError, match="URL length exceeds maximum allowed"):
        validate_url(url)

def test_url_tld_ipv6_handling_expected() -> None:
    """Ensure validate_url uses parsed.hostname for localhost checks on IPv6."""
    url = "http://[::1]"
    assert validate_url(url) == url

def test_url_credentials_bypassing_tld_check_expected() -> None:
