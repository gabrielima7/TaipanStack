...
def test_taipanstack_script_extra_handle_error_standard_expected(capsys):
...
def test_taipanstack_script_extra_run_command_file_not_found_standard_expected():
...
def test_taipanstack_script_extra_run_command_called_process_error_standard_expected():
    args = argparse.Namespace(dry_run=False, verbose=False)
...
def test_taipanstack_script_extra_safe_write_rename_error_standard_expected(tmp_path):
...
def test_taipanstack_script_extra_safe_write_write_error_standard_expected(tmp_path):
    args = argparse.Namespace(dry_run=False, verbose=False, force=False)
    file_path = tmp_path / "test.txt"
    with (
...
def test_taipanstack_script_extra_generate_pyproject_config_dry_run_with_config_standard_expected(
