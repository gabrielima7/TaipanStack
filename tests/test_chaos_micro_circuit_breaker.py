...
def test_chaos_circuit_breaker_get_valid_elapsed_type_mutation(monkeypatch: pytest.MonkeyPatch):
    ...
    try:
        ...
        assert elapsed is None or elapsed == 50.0  # elapsed could be calculated if isfinite doesn't fail
    except Exception as e:
        pytest.fail(f"CircuitBreaker crashed with exception: {e}")

def test_chaos_circuit_breaker_get_valid_elapsed_type_mutation_not_isfinite(monkeypatch: pytest.MonkeyPatch):
    """Simulate time.monotonic returning a malicious type."""
    class CorruptFloat(float):
        def __sub__(self, other):
            raise RuntimeError("Chaos __sub__")
...
def test_chaos_circuit_breaker_get_valid_elapsed_type_mutation_isfinite_runtime(monkeypatch: pytest.MonkeyPatch):
    """Simulate time.monotonic returning a malicious type."""
    class CorruptFloat(float):
        def __ge__(self, other):
            raise RuntimeError("Chaos __ge__")
