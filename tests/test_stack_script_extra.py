...
def test_run_command_file_not_found_expected():
    args = argparse.Namespace(dry_run=False, verbose=False)
    with patch("subprocess.run", side_effect=FileNotFoundError), patch("taipanstack_bootstrapper._handle_error") as mock_error:
...
def test_run_command_called_process_error():
    ...
    with patch("subprocess.run", side_effect=err), patch("taipanstack_bootstrapper._handle_error") as mock_error:
...
def test_safe_write_rename_error(tmp_path):
    ...
    with patch("pathlib.Path.rename", side_effect=PermissionError), patch("taipanstack_bootstrapper._handle_error") as mock_error:
...
def test_safe_write_write_error(tmp_path):
    args = argparse.Namespace(dry_run=False, verbose=False, force=False)
    file_path = tmp_path / "test.txt"
    with patch("pathlib.Path.write_text", side_effect=PermissionError), patch("taipanstack_bootstrapper._handle_error") as mock_error:
        taipanstack._safe_write(file_path, "new", args)
        mock_error.assert_called_once()
        assert "Could not write to file" in mock_error.call_args[0][0]

def test_generate_pyproject_config_dry_run_with_config_expected(tmp_path, monkeypatch):
    ...
    with patch("taipanstack_bootstrapper._log") as mock_log:
        ...
        mock_log.assert_any_call("Would add tool configurations to pyproject.toml", args, is_verbose=True)
