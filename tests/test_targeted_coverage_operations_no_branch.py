import pytest
from taipanstack.core.optimizations import OptimizationProfile, OptimizationResult
...
def test_apply_optimizations_nothing_skipped(monkeypatch):
    ...
    monkeypatch.setattr(opts, "get_features", lambda *args, **kwargs: MockFeatures())
...
def test_process_path_part_empty_result(monkeypatch):
    ...
    monkeypatch.setattr(san, "_is_safe_path_part", lambda p, s: False)
    monkeypatch.setattr(san, "sanitize_filename", lambda p, preserve_extension: "")
