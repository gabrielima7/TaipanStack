...
class TestResultToResponse:
    ...
    def test_custom_status_codes_expected(self) -> None:
...
class TestTaipanMiddleware:
    ...
    async def test_unhandled_exception_returns_500_expected(self) -> None:
        ...
        mw = TaipanMiddleware(_make_crashing_app, security_headers=False)
        capture = _ResponseCapture()
        await mw({"type": "http"}, AsyncMock(), capture)
...
class TestSendJsonResponse:
    ...
    async def test_bridge_web_sends_json_expected(self) -> None:
        ...
        capture = _ResponseCapture()
        await _send_json_response(capture, status=201, body={"created": True})
