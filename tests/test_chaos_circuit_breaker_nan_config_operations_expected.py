...
def test_chaos_circuit_breaker_config_rejects_nan_failure_threshold_expected():
...
def test_chaos_circuit_breaker_config_rejects_nan_success_threshold_expected():
    """Chaos test: Inject NaN for success_threshold in CircuitBreakerConfig."""
    with pytest.raises(ValueError, match="finite"):
        CircuitBreakerConfig(success_threshold=float("nan"))


def test_chaos_circuit_breaker_config_rejects_nan_timeout_expected():
