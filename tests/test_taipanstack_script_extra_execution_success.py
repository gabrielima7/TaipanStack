...
def test_taipanstack_script_extra_handle_error_execution_success(capsys):
...
def test_taipanstack_script_extra_run_command_file_not_found_execution_success():
...
def test_taipanstack_script_extra_run_command_called_process_error_execution_success():
    args = argparse.Namespace(dry_run=False, verbose=False)
...
def test_taipanstack_script_extra_safe_write_rename_error_execution_success(tmp_path):
...
def test_taipanstack_script_extra_safe_write_write_error_execution_success(tmp_path):
    args = argparse.Namespace(dry_run=False, verbose=False, force=False)
    file_path = tmp_path / "test.txt"
    with (
...
def test_taipanstack_script_extra_generate_pyproject_config_dry_run_with_config_execution_success(
