...
class TestParseEnv:
    ...
    def test_skips_comments_and_blanks_expected(self) -> None:
    ...
    def test_skips_lines_without_equals_expected(self) -> None:
...
class TestParseJson:
    ...
    def test_non_dict_json_expected(self) -> None:
...
class TestHashFile:
    ...
    def test_hash_existing_file_expected(self, tmp_path: Path) -> None:
        """Returns Ok with hash for existing file."""
        f = tmp_path / "test.txt"
        f.write_text("hello")
        result = _hash_file(f)
        assert isinstance(result, Ok)
        assert len(result.ok_value) == 64

    def test_hash_missing_file_expected(self, tmp_path: Path) -> None:
...
class TestLoadFileData:
    ...
    def test_load_json_file_expected(self, tmp_path: Path) -> None:
    ...
    def test_load_env_file_expected(self, tmp_path: Path) -> None:
...
class TestConfigWatcher:
    ...
    @pytest.mark.asyncio
    async def test_start_stop_lifecycle_expected(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_detects_file_change_expected(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_missing_file_does_not_crash_expected(self, tmp_path: Path) -> None:
    ...
    def test_detect_changes_no_change_expected(self, tmp_path: Path) -> None:
    ...
    def test_validate_and_apply_with_env_file_expected(self, tmp_path: Path) -> None:
    ...
    def test_validate_and_apply_missing_file_expected(self, tmp_path: Path) -> None:
        """_validate_and_apply returns Err when file can't be loaded."""
        watcher = ConfigWatcher(config_paths=[], config_model=SampleConfig)
