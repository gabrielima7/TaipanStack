...
from taipanstack.security.validators import MAX_EMAIL_LOCAL_LENGTH, MAX_EMAIL_DOMAIN_LENGTH, validate_email
...
@given(st.text(min_size=MAX_EMAIL_LOCAL_LENGTH + MAX_EMAIL_DOMAIN_LENGTH + 10, max_size=MAX_EMAIL_LOCAL_LENGTH + MAX_EMAIL_DOMAIN_LENGTH + 1000))
def test_fuzz_email_massive_strings_dos(email: str) -> None:
    ...
    with pytest.raises(ValueError, match="Email length exceeds maximum allowed|Invalid email format|Email local part exceeds|Email domain exceeds"):
...
def test_fuzz_email_null_bytes(email: str) -> None:
    ...
    with pytest.raises(ValueError, match="Email contains invalid characters|Invalid email format"):
        validate_email(email)
