...
class TestSafeCommandResult:
    ...
    def test_duration_seconds_default_expected(self) -> None:
...
class TestDefaultAllowedCommands:
    ...
    def test_contains_essential_commands_expected(self) -> None:
...
class TestRunSafeCommand:
    ...
    def test_run_echo_command_expected(self) -> None:
    ...
    def test_dry_run_mode_expected(self) -> None:
    ...
    def test_command_not_in_whitelist_expected(self) -> None:
        """Test that commands not in whitelist are rejected."""
        with pytest.raises(SecurityError, match="Command not in allowed list"):
            run_safe_command(["dangerous_command"], allowed_commands=["safe_command"])

    def test_empty_command_rejected_expected(self) -> None:
        """Test that empty command raises SecurityError."""
        with pytest.raises(SecurityError, match="Empty command"):
            run_safe_command([])

    def test_command_injection_blocked_expected(self) -> None:
    ...
    def test_utils_subprocess_working_directory_expected(self, tmp_path: Path) -> None:
        ...
        result = run_safe_command(["echo", "hello"], cwd=tmp_path)
        assert result.success is True

    def test_invalid_working_directory_expected(self) -> None:
    ...
    def test_custom_allowed_commands_expected(self) -> None:
        """Test custom allowed commands list."""
        result = run_safe_command(["echo", "custom"], allowed_commands=["echo"])
        assert result.success is True

    def test_duration_is_tracked_expected(self) -> None:
