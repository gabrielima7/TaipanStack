...
def test_bench_sanitize_string_xss_expected(benchmark: BenchmarkFixture) -> None:
    ...
    benchmark(
        sanitize_string, "<script>alert('xss')</script><img onerror=alert(1) src=x>"
    )
...
def test_bench_sanitize_env_value_standard_expected(
    benchmark: BenchmarkFixture,
) -> None:
    """Benchmark sanitize_env_value with typical env content."""
    benchmark(
        sanitize_env_value, "DATABASE_URL=postgresql://user:pass@localhost:5432/db"
    )
...
def test_bench_sanitize_sql_identifier_dirty_expected(
    benchmark: BenchmarkFixture,
) -> None:
