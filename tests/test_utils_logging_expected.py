...
class TestStackLogger:
    ...
    def test_utils_logging_logging_no_structlog_not_installed(self) -> None:
    ...
    def test_utils_logging_init_with_defaults_with_defaults(self) -> None:
        """Test logger initialization with defaults."""
        logger = StackLogger()
        assert logger.name == "stack"
        assert logger.level == "INFO"

    def test_utils_logging_init_with_custom_name_custom_name(self) -> None:
        """Test logger with custom name."""
        logger = StackLogger(name="my_module")
        assert logger.name == "my_module"

    def test_utils_logging_init_with_custom_level_custom_level(self) -> None:
        """Test logger with custom level."""
        logger = StackLogger(level="DEBUG")
        assert logger.level == "DEBUG"

    def test_utils_logging_bind_adds_context_adds_context(self) -> None:
    ...
    def test_utils_logging_bind_returns_self_for_chaining(self) -> None:
        """Test that bind returns self for chaining."""
        logger = StackLogger()
        result = logger.bind(key="value")
        assert result is logger

    def test_utils_logging_unbind_removes_context_context_keys(self) -> None:
    ...
    def test_utils_logging_unbind_missing_key_ok_nonexistent_keys(self) -> None:
        """Test that unbind works for non-existent keys."""
        logger = StackLogger()
        logger.unbind("nonexistent")  # Should not raise

    def test_utils_logging_unbind_returns_self_for_chaining(self) -> None:
        """Test that unbind returns self for chaining."""
        logger = StackLogger()
        result = logger.unbind("key")
        assert result is logger

    def test_utils_logging_debug_logging_debug_logging(
    ...
    def test_utils_logging_info_logging_info_logging(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test info logging."""
    ...
    def test_utils_logging_warning_logging_warning_logging(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_error_logging_error_logging(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        """Test error logging."""
    ...
    def test_utils_logging_critical_logging_critical_logging(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_exception_logging_with_traceback(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_context_in_message_log_message(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestStackLoggerStructured:
    ...
    def test_utils_logging_setup_logging_structured_with_structlog(self) -> None:
        """Test setup_logging with structlog."""
        setup_logging(use_structured=True)
        # Verify it doesn't break

    def test_utils_logging_get_logger_structured_with_structlog(self) -> None:
...
class TestMaskSensitiveDataProcessor:
    ...
    def test_utils_logging_mask_sensitive_data_expected(self) -> None:
        event_dict = {
            "message": "User login",
...
class TestGetLogger:
    ...
    def test_utils_logging_returns_stack_logger_stacklogger_instance(self) -> None:
        """Test that get_logger returns StackLogger instance."""
        logger = get_logger()
        assert isinstance(logger, StackLogger)

    def test_utils_logging_custom_name_custom_name(self) -> None:
        """Test logger with custom name."""
        logger = get_logger(name="custom")
        assert logger.name == "custom"

    def test_utils_logging_custom_level_custom_level(self) -> None:
        """Test logger with custom level."""
...
class TestSetupLogging:
    ...
    def test_utils_logging_setup_with_defaults_default_parameters(self) -> None:
        """Test setup_logging with default parameters."""
        setup_logging()
        # Should not raise

    def test_utils_logging_setup_with_debug_level_debug_level(self) -> None:
        """Test setup with DEBUG level."""
        setup_logging(level="DEBUG")

    def test_utils_logging_setup_with_simple_format_simple_format(self) -> None:
        """Test setup with simple format."""
        setup_logging(format_type="simple")

    def test_utils_logging_setup_with_json_format_json_format(self) -> None:
        """Test setup with JSON format."""
        setup_logging(format_type="json")

    def test_utils_logging_setup_with_detailed_format_detailed_format(self) -> None:
        """Test setup with detailed format."""
        setup_logging(format_type="detailed")

    def test_utils_logging_setup_with_log_file_log_file(self, tmp_path: Path) -> None:
...
class TestLogOperation:
    ...
    def test_utils_logging_exceptions_handling_as_failures(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_logs_start_and_end_are_logged(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_logs_duration_is_logged(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_custom_logger_custom_logger(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_logs_exception_on_failure_on_failure(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_reraises_exception_are_reraised(self) -> None:
        """Test that exceptions are re-raised."""
        with pytest.raises(ValueError, match="original"):
            with log_operation("test"):
                raise ValueError("original")

    def test_utils_logging_exceptions_caught_and_logged_and_logged(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_utils_logging_unexpected_exceptions_bypass_catch_not_logged(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestFormatConstants:
    ...
    def test_utils_logging_default_format_has_required_fields_required_placeholders(
        self,
    ) -> None:
    ...
    def test_utils_logging_json_format_is_valid_json_template_json_structure(
        self,
    ) -> None:
...
class TestCorrelationId:
    ...
    def test_utils_logging_correlation_id_processor_with_id_when_set(
        self,
    ) -> None:
    ...
    def test_utils_logging_correlation_id_processor_without_id_not_set(
        self,
    ) -> None:
...
def test_utils_logging_is_sensitive_regex_none_returns_false_is_none() -> None:
...
def test_utils_logging_redact_custom_mutable_mapping_a_dict() -> None:
