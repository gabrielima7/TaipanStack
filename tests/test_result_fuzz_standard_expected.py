...
@given(st.text())
def test_result_ok_fuzz_standard_expected(value: str) -> None:
    res = Ok(value)
...
@given(st.text())
def test_result_err_fuzz_standard_expected(value: str) -> None:
...
def test_result_safe_fuzz_standard_expected(values: list[object]) -> None:
    @safe
...
@given(st.lists(st.one_of(st.text(), st.integers())))
def test_result_safe_from_fuzz_standard_expected(values: list[object]) -> None:
    @safe_from(TypeError)
    def maybe_fail(v: object) -> str:
        if isinstance(v, int):
            raise TypeError("Int not allowed")
