...
class TestSecurityConfig:
    ...
    def test_config_models_default_values_expected(self) -> None:
    ...
    def test_config_models_paranoid_level_expected(self) -> None:
        """Test paranoid security level."""
        config = SecurityConfig(level="paranoid")
        assert config.level == "paranoid"

    def test_config_models_frozen_config_expected(self) -> None:
        """Test that config is immutable."""
        config = SecurityConfig()
        with pytest.raises(ValidationError):
            config.level = "standard"

    def test_extra_fields_forbidden_expected(self) -> None:
...
class TestLoggingConfig:
    ...
    def test_config_models_valid_levels_expected(self) -> None:
        """Test all valid log levels."""
        for level in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
            config = LoggingConfig(level=level)
            assert config.level == level

    def test_invalid_level_rejected_expected(self) -> None:
...
class TestStackConfig:
    ...
    def test_valid_project_name_expected(self) -> None:
        """Test valid project names."""
        config = StackConfig(project_name="my_awesome_project")
        assert config.project_name == "my_awesome_project"

    def test_invalid_project_name_rejected_expected(self) -> None:
    ...
    def test_valid_python_version_expected(self) -> None:
        """Test valid Python versions."""
        config = StackConfig(python_version="3.11")
        assert config.python_version == "3.11"

    def test_invalid_python_version_rejected_expected(self) -> None:
    ...
    def test_valid_project_dir_expected(self, tmp_path: Path) -> None:
    ...
    def test_path_traversal_rejected_expected(self, tmp_path: Path) -> None:
        """Test path traversal in project_dir is rejected."""
        with pytest.raises(ValidationError, match="Path traversal"):
            StackConfig(project_dir=Path("../../../etc"))

    def test_absolute_path_accepted_expected(self, tmp_path: Path) -> None:
    ...
    def test_non_existent_path_accepted_expected(self, tmp_path: Path) -> None:
    ...
    def test_direct_validate_project_dir_call_expected(self) -> None:
    ...
    def test_validate_project_dir_traversal_direct_call_expected(self) -> None:
        """Test direct call to validate_project_dir with traversal."""
        with pytest.raises(ValueError, match="Path traversal"):
            StackConfig.validate_project_dir(Path("safe/../unsafe"))

    def test_path_with_double_dots_in_name_rejected_expected(self) -> None:
    ...
    def test_paranoid_requires_all_tools_expected(self) -> None:
        """Test paranoid mode requires all security tools."""
        with pytest.raises(ValidationError, match="requires all security tools"):
            StackConfig(security=SecurityConfig(level="paranoid", enable_bandit=False))

    def test_to_target_version_expected(self) -> None:
