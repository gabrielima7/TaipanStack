...
from app.secure_system import InMemoryUserRepository, UserAlreadyExistsError, UserInDB
...
def test_in_memory_user_repository_update_conflict():
    ...
    user1 = UserInDB(
        id=uuid4(), username="user1", email="user1@example.com", password_hash="hash"
    )
    user2 = UserInDB(
        id=uuid4(), username="user2", email="user2@example.com", password_hash="hash"
    )
    ...
    user1_updated_username = UserInDB(
        id=user1.id, username="user2", email="user1@example.com", password_hash="hash"
    )
    ...
    user1_updated_email = UserInDB(
        id=user1.id, username="user1", email="user2@example.com", password_hash="hash"
    )
...
def test_in_memory_user_repository_new_user_conflict():
    ...
    user2 = UserInDB(
        id=uuid4(), username="user1", email="user2@example.com", password_hash="hash"
    )
    ...
    # Conflict on email
    user3 = UserInDB(
        id=uuid4(), username="user3", email="user1@example.com", password_hash="hash"
    )
...
def test_in_memory_user_repository_update_conflict_both():
    ...
    user1_updated = UserInDB(
        id=user1.id, username="user2", email="user2@example.com", password_hash="hash"
    )
...
def test_in_memory_user_repository_update_conflict_both_same():
    ...
    user1_updated = UserInDB(
        id=user1.id, username="user1", email="user1@example.com", password_hash="hash"
    )
    result = repo.save(user1_updated)
