...
nasty_text = st.text(alphabet=st.characters(codec="utf-8"), min_size=0, max_size=500)
nasty_filename = st.text(alphabet=st.characters(whitelist_categories=("L", "N", "P", "S", "Z"), whitelist_characters='../\\<>:"|?*\x00'), min_size=0, max_size=300)
nasty_path = st.text(alphabet=st.characters(whitelist_categories=("L", "N"), whitelist_characters="/..\\\x00"), min_size=1, max_size=200)
FUZZ_EXAMPLES = 1000
...
class TestSanitizeStringProperties:
    ...
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_never_contains_null_bytes_expected(self, text: str) -> None:
    ...
    @given(text=nasty_text)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_control_characters_except_whitespace_expected(self, text: str) -> None:
        """Output must not contain control characters (except newline, tab, CR)."""
        result = sanitize_string(text)
        allowed_controls = {"\n", "\r", "\t"}
        for ch in result:
            if ord(ch) < 32 and ch not in allowed_controls:
                pytest.fail(f"Control character U+{ord(ch):04X} found in output: {result!r}")

    @given(text=nasty_text, max_len=st.integers(min_value=1, max_value=100))
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_respects_max_length_expected(self, text: str, max_len: int) -> None:
    ...
    @given(text=nasty_text)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_returns_string_type_expected(self, text: str) -> None:
...
class TestSanitizeFilenameProperties:
    ...
    @given(text=nasty_filename)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_path_separators_expected(self, text: str) -> None:
    ...
    @given(text=nasty_filename)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_null_bytes_expected(self, text: str) -> None:
    ...
    @given(text=st.text(alphabet=st.characters(whitelist_categories=("L", "N", "P", "S", "Z"), whitelist_characters='../\\<>:"|?*\x00'), min_size=1, max_size=300), max_len=st.integers(min_value=5, max_value=255))
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_respects_max_length_expected(self, text: str, max_len: int) -> None:
    ...
    @given(text=nasty_filename)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_windows_reserved_chars_expected(self, text: str) -> None:
        ...
        reserved = set('<>:"|?*')
        for ch in stem:
            assert ch not in reserved, f"Reserved char {ch!r} found in stem {stem!r} (full: {result!r})"

    def test_mutmut_boundaries_and_limits_expected(self) -> None:
...
class TestSanitizePathProperties:
    ...
    @given(text=st.from_regex("[a-zA-Z0-9_./]{1,50}", fullmatch=True))
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_traversal_in_output_expected(self, text: str) -> None:
        ...
        try:
            ...
            for part in Path(str(result)).parts:
                assert part != "..", f"Traversal component '..' found as path part in: {result!r}"
        except ValueError:
            pass
    ...
    @given(segments=st.lists(st.from_regex("[a-z]{1,8}", fullmatch=True), min_size=1, max_size=20), max_depth=st.integers(min_value=1, max_value=10))
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_property_sanitizers_depth_enforcement_expected(self, segments: list[str], max_depth: int) -> None:
...
class TestSanitizeEnvValueProperties:
    ...
    @given(text=nasty_text)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_no_newlines_by_default_expected(self, text: str) -> None:
    ...
    def test_returns_string_type_expected(self, text: str) -> None:
        ...
        result = sanitize_env_value(text)
        assert isinstance(result, str)
sql_text = st.text(alphabet=st.characters(codec="utf-8"), min_size=1, max_size=200)
valid_sql_text = st.text(alphabet=st.sampled_from(list(string.ascii_letters + string.digits + "_")), min_size=1, max_size=200)
...
class TestSanitizeSqlIdentifierProperties:
    ...
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_only_valid_chars_in_output_expected(self, text: str) -> None:
    ...
    @given(text=valid_sql_text)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_max_length_128_expected(self, text: str) -> None:
    ...
    @given(text=valid_sql_text)
    @settings(max_examples=FUZZ_EXAMPLES, suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors])
    def test_does_not_start_with_digit_expected(self, text: str) -> None:
    ...
    def test_only_invalid_chars_raises_expected(self) -> None:
