...
async def test_orchestrator_match():
    ...
    with patch(
        "taipanstack.resilience.adaptive.orchestrator.ResilienceOrchestrator._execute_with_timeout",
...
def test_config_watcher_match():
    ...
    with patch(
        "taipanstack.resilience.watchdogs.config_watcher._load_file_data",
        return_value="Not a result",
    ):
        with pytest.raises(TypeError):
            watcher._validate_and_apply(Path("test"))
    with patch(
        "taipanstack.resilience.watchdogs.config_watcher._hash_file",
        return_value="Not a result",
    ):
        with pytest.raises(TypeError):
    ...
    with (
        patch(
            "taipanstack.resilience.watchdogs.config_watcher._load_file_data",
            return_value=Ok({"dummy": "data"}),
        ),
        patch(
            "taipanstack.resilience.watchdogs.config_watcher.validate_config",
            return_value="Not a result",
        ),
    ):
        with pytest.raises(TypeError):
...
async def test_health_pinger_match():
    ...
    with patch(
        "taipanstack.resilience.watchdogs.health_pinger.check_target",
        return_value="Not a result",
    ):
        with pytest.raises(TypeError):
...
def test_security_guards_match():
    with patch(
        "taipanstack.security.guards._validate_ssrf_url", return_value="Not a result"
    ):
        with pytest.raises(TypeError):
            guard_ssrf("http://test")
    with patch(
        "taipanstack.security.guards._check_ip_safety", return_value="Not a result"
    ):
        ...
        with patch(
            "taipanstack.security.guards._validate_ssrf_url",
            return_value=Ok("hostname"),
        ):
            with pytest.raises(TypeError):
                guard_ssrf("http://test")
