...
def test_chaos_retry_result_retry_result_monad_chaos_standard_expected():
    attempts = 0
...
def test_chaos_retry_result_retry_result_monad_chaos_coverage_standard_expected():
    attempts = 0
...
def test_chaos_retry_result_retry_result_monad_chaos_sync_not_on_standard_expected():
    @retry(max_attempts=3, on=(ValueError,))
    def failing_func():
...
def test_chaos_retry_result_retry_result_monad_chaos_exhaust_sync_standard_expected():
    @retry(max_attempts=2, on=(ValueError,))
    def failing_func():
        return Err(ValueError("Chaos failure wrapped in Result"))
