...
def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_nan_success_threshold_expected():
...
def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_inf_success_threshold_expected():
...
def test_chaos_circuit_breaker_nan_bulkhead_rejects_nan_timeout_expected():
    """Chaos test: Inject NaN for timeout."""
    with pytest.raises(ValueError, match="must be a finite"):
        Bulkhead("default", timeout=float("nan"))


def test_chaos_circuit_breaker_nan_orchestrator_rejects_nan_timeout_expected():
    """Chaos test: Inject NaN for orchestrator timeout."""
    with pytest.raises(ValueError, match="must be a finite"):
        ResilienceOrchestrator().with_timeout(float("nan"))


def test_chaos_circuit_breaker_nan_orchestrator_with_bulkhead_rejects_nan_timeout_expected():
