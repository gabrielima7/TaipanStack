...
def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_nan_failure_threshold_for_failurethreshold():
...
def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_nan_success_threshold_for_successthreshold():
    """Chaos test: Inject NaN for success_threshold."""
    with pytest.raises(ValueError, match="must be a finite"):
        CircuitBreaker(success_threshold=float("nan"))


def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_inf_failure_threshold_for_failurethreshold():
    """Chaos test: Inject Inf for failure_threshold."""
    with pytest.raises(ValueError, match="must be a finite"):
        CircuitBreaker(failure_threshold=float("inf"))


def test_chaos_circuit_breaker_nan_circuit_breaker_rejects_inf_success_threshold_for_successthreshold():
    """Chaos test: Inject Inf for success_threshold."""
    with pytest.raises(ValueError, match="must be a finite"):
        CircuitBreaker(success_threshold=float("inf"))


def test_chaos_circuit_breaker_nan_adaptive_breaker_rejects_nan_recovery_timeout_recovery_timeout():
    """Chaos test: Inject NaN for recovery timeout."""
    with pytest.raises(ValueError, match="must be a finite"):
        AdaptiveCircuitBreaker(recovery_timeout=float("nan"))


def test_chaos_circuit_breaker_nan_bulkhead_rejects_nan_timeout_for_timeout():
    """Chaos test: Inject NaN for timeout."""
    with pytest.raises(ValueError, match="must be a finite"):
        Bulkhead("default", timeout=float("nan"))


def test_chaos_circuit_breaker_nan_orchestrator_rejects_nan_timeout_orchestrator_timeout():
    """Chaos test: Inject NaN for orchestrator timeout."""
    with pytest.raises(ValueError, match="must be a finite"):
        ResilienceOrchestrator().with_timeout(float("nan"))


def test_chaos_circuit_breaker_nan_orchestrator_with_bulkhead_rejects_nan_timeout_bulkhead_timeout():
