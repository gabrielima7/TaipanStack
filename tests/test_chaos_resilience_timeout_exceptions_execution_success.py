...
@pytest.mark.asyncio
async def test_chaos_timeout_async_cancelled_error_execution_success():
...
@pytest.mark.asyncio
async def test_chaos_timeout_async_overflow_error_execution_success():
    def side_effect(*args, **kwargs):
        # Prevent "coroutine was never awaited" warning
        args[0].close()
...
def test_chaos_timeout_sync_overflow_error_execution_success():
    with patch(
