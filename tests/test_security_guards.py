...
class TestGuardPathTraversal:
    ...
    def test_safe_path_passes_expected(self, tmp_path: Path) -> None:
    ...
    def test_relative_path_within_base_expected(self, tmp_path: Path) -> None:
    ...
    def test_double_dot_blocked_expected(self, tmp_path: Path) -> None:
        """Test that .. patterns are blocked."""
        with pytest.raises(SecurityError):
            guard_path_traversal("foo/../../../etc/passwd", tmp_path)

    def test_url_encoded_traversal_blocked_expected(self, tmp_path: Path) -> None:
        """Test that URL encoded traversal is blocked."""
        with pytest.raises(SecurityError):
            guard_path_traversal("%2e%2e/etc/passwd", tmp_path)

    def test_path_escapes_base_dir_expected(self, tmp_path: Path) -> None:
    ...
    def test_symlinks_blocked_by_default_expected(self, tmp_path: Path) -> None:
    ...
    def test_symlinks_allowed_when_enabled_expected(self, tmp_path: Path) -> None:
    ...
    def test_path_escapes_base_dir_msg_expected(self, tmp_path: Path) -> None:
...
class TestGuardCommandInjection:
    ...
    def test_safe_command_passes_expected(self) -> None:
        """Test that safe commands pass validation."""
        cmd = ["python", "-m", "pytest", "-v"]
        result = guard_command_injection(cmd)
        assert result == cmd

    def test_empty_command_blocked_expected(self) -> None:
    ...
    def test_dollar_expansion_blocked_expected(self) -> None:
        """Test that dollar expansion is blocked."""
        with pytest.raises(SecurityError):
            guard_command_injection(["echo", "$(whoami)"])

    def test_allowed_commands_whitelist_expected(self) -> None:
        """Test command whitelist functionality."""
        cmd = ["python", "-c", "print('hello')"]
        result = guard_command_injection(cmd, allowed_commands=["python"])
        assert result == cmd

    def test_command_not_in_whitelist_blocked_expected(self) -> None:
    ...
    def test_empty_allowed_commands_whitelist_blocks_all_expected(self) -> None:
...
class TestGuardFileExtension:
    ...
    def test_safe_extension_passes_expected(self) -> None:
    ...
    def test_normalize_ext_in_allowed_expected(self) -> None:
