...
def test_adaptive_breaker_evaluate_result_expected():
...
def test_adaptive_breaker_reset_and_should_allow_expected():
...
def test_adaptive_breaker_invalid_recovery_timeout_expected():
    with pytest.raises(
...
def test_adaptive_breaker_evaluate_trip_returns_early_expected():
...
def test_adaptive_breaker_half_open_success_and_failure_expected(monkeypatch):
...
def test_adaptive_breaker_evaluate_trip_with_half_open_expected(monkeypatch):
    breaker = AdaptiveCircuitBreaker(recovery_timeout=0.0)
    breaker._state = CircuitState.OPEN
    breaker._last_opened_at = 0.0
...
def test_adaptive_breaker_evaluate_trip_failure_with_half_open_expected(monkeypatch):
    breaker = AdaptiveCircuitBreaker(recovery_timeout=30.0)
    breaker._state = CircuitState.HALF_OPEN
...
def test_adaptive_breaker_evaluate_trip_with_min_throughput_expected():
...
def test_adaptive_breaker_evaluate_trip_with_min_throughput_and_target_rate_expected():
