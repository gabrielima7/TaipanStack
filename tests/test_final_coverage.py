...
class TestLoggingComplete:
    ...
    def test_logger_all_levels_expected(self) -> None:
    ...
    def test_log_operation_context_manager_expected(self) -> None:
...
class TestGuardsComplete:
    ...
    def test_guard_env_variable_pattern_not_in_allowed_expected(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_guard_env_variable_pattern_in_allowed_expected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test guard_env_variable when pattern matches and in allowed."""
        from taipanstack.security.guards import guard_env_variable
        monkeypatch.setenv("CUSTOM_API_KEY", "allowed_secret")
        result = guard_env_variable("CUSTOM_API_KEY", allowed_names=["CUSTOM_API_KEY"])
        assert result == "allowed_secret"

class TestSubprocessComplete:
    """Complete tests for subprocess module."""

    def test_run_safe_command_with_all_options_expected(self) -> None:
        """Test run_safe_command with all options."""
        from taipanstack.utils.subprocess import run_safe_command
        result = run_safe_command(["echo", "hello"], timeout=30.0, capture_output=True, check=False)
...
class TestSanitizersComplete:
    ...
    def test_sanitize_string_with_unicode_expected(self) -> None:
    ...
    def test_sanitize_string_with_max_length_expected(self) -> None:
        """Test sanitize_string with max_length."""
        from taipanstack.security.sanitizers import sanitize_string
        result = sanitize_string("This is a long string", max_length=10)
        assert len(result) == 10

    def test_sanitize_filename_long_name_expected(self) -> None:
    ...
    def test_sanitize_env_value_multiline_expected(self) -> None:
...
class TestFilesystemComplete:
    ...
    def test_safe_write_non_atomic_expected(self, tmp_path: Path) -> None:
...
class TestConfigModelsComplete:
    ...
    def test_stack_config_all_options_expected(self) -> None:
        """Test StackConfig with all options."""
        from taipanstack.config.models import StackConfig
        config = StackConfig(project_name="myproject", python_version="3.11", dry_run=True, force=True)
...
class TestDecoratorsComplete:
    ...
    def test_validate_inputs_with_validation_expected(self) -> None:
    ...
    def test_guard_exceptions_reraise_non_security_expected(self) -> None:
    ...
    def test_deprecated_with_removal_version_expected(self) -> None:
    ...
    def test_require_type_passes_expected(self) -> None:
...
class TestCircuitBreakerComplete:
    ...
    def test_excluded_exceptions_work_expected(self) -> None:
...
class TestRetryComplete:
    ...
    def test_retry_backoff_exponential_expected(self) -> None:
