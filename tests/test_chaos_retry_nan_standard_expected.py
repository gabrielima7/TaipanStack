...
def test_chaos_retry_nan_chaos_retry_rejects_nan_max_attempts_standard_expected():
...
def test_chaos_retry_nan_chaos_retry_rejects_nan_initial_delay_standard_expected():
...
def test_chaos_retry_nan_chaos_retrier_rejects_nan_max_attempts_standard_expected():
    """Chaos test: Inject NaN for max_attempts in Retrier."""
    with pytest.raises(ValueError, match="finite"):
        Retrier(max_attempts=float("nan"))


def test_chaos_retry_nan_chaos_retrier_rejects_nan_initial_delay_standard_expected():
    """Chaos test: Inject NaN for initial_delay in Retrier."""
    with pytest.raises(ValueError, match="finite"):
        Retrier(initial_delay=float("nan"))


def test_chaos_retry_nan_chaos_retry_on_exception_rejects_nan_max_attempts_standard_expected():
...
def test_chaos_retry_nan_chaos_retry_config_rejects_nan_max_delay_standard_expected():
    """Chaos test: Inject NaN for max_delay in RetryConfig."""
    with pytest.raises(ValueError, match="finite"):
        RetryConfig(max_delay=float("nan"))


def test_chaos_retry_nan_chaos_retry_config_rejects_inf_exponential_base_standard_expected():
    """Chaos test: Inject Inf for exponential_base in RetryConfig."""
    with pytest.raises(ValueError, match="finite"):
        RetryConfig(exponential_base=float("inf"))


def test_chaos_retry_nan_chaos_retry_config_rejects_nan_jitter_factor_standard_expected():
