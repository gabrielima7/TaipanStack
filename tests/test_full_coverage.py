...
class TestLoggingImportFallback:
    ...
    def test_has_structlog_constant_exists_expected(self) -> None:
        """Verify HAS_STRUCTLOG constant is defined."""
        assert isinstance(HAS_STRUCTLOG, bool)

    def test_stack_logger_without_structlog_expected(self) -> None:
    ...
    def test_setup_logging_without_structlog_expected(self) -> None:
...
class TestValidatorsPythonVersionEdgeCases:
    ...
    def test_validate_python_version_alpha_suffix_expected(self) -> None:
        """Test version with non-numeric characters fails at regex."""
        with pytest.raises(ValueError, match="Invalid version format"):
            validate_python_version("3.12a")

    def test_validate_python_version_too_many_parts_expected(self) -> None:
...
class TestGuardsEnvVariableAllowedBranch:
    ...
    def test_guard_env_variable_sensitive_pattern_allowed_expected(self) -> None:
        """Test sensitive pattern allowed when explicitly in allowed list."""
        with mock.patch.dict(os.environ, {"MY_API_TOKEN": "secret123"}):
            result = guard_env_variable("MY_API_TOKEN", allowed_names=["MY_API_TOKEN"])
            assert result == "secret123"

    def test_guard_env_variable_sensitive_pattern_not_in_allowed_expected(self) -> None:
...
class TestSanitizersFilenameEdgeCases:
    ...
    def test_sanitize_filename_no_room_for_stem_expected(self) -> None:
        """Test filename truncation when extension is longer than max."""
        result = sanitize_filename("abc.toolongext", max_length=4)
        assert len(result) <= 4

    def test_sanitize_filename_extension_equals_max_expected(self) -> None:
...
class TestSanitizersPathEdgeCases:
    ...
    def test_sanitize_path_with_base_dir_relative_expected(self) -> None:
...
class TestFilesystemSizeLimit:
    ...
    def test_safe_read_no_size_limit_expected(self) -> None:
...
class TestFilesystemEnsureDir:
    ...
    def test_ensure_dir_nested_expected(self) -> None:
...
class TestRetryReraiseFlow:
    ...
    def test_retry_reraise_false_still_raises_expected(self) -> None:
...
class TestConfigModelsValidation:
    ...
    def test_stack_config_invalid_python_version_expected(self) -> None:
        """Test StackConfig with invalid Python version format."""
        with pytest.raises(ValueError, match="invalid"):
            StackConfig(python_version="3.x")

    def test_stack_config_old_python_expected(self) -> None:
