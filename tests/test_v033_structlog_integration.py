...
class TestRetryStructlogIntegration:
    ...
    def test_retry_calls_structlog_warning_on_retry_without_callback_expected(self) -> None:
        """When no on_retry callback is provided, structlog.warning is called."""
        mock_structlog_logger = MagicMock()
        call_count = 0
        with patch("taipanstack.resilience.retry._HAS_STRUCTLOG", True), patch("taipanstack.resilience.retry._structlog_logger", mock_structlog_logger), patch("taipanstack.resilience.retry.time.sleep"):
    ...
    def test_retry_does_not_call_structlog_when_callback_provided_expected(self) -> None:
        ...
        with patch("taipanstack.resilience.retry._HAS_STRUCTLOG", True), patch("taipanstack.resilience.retry._structlog_logger", mock_structlog_logger), patch("taipanstack.resilience.retry.time.sleep"):
            ...
            @retry(max_attempts=2, initial_delay=0.0, jitter=False, on_retry=lambda a, m, e, d: callback_calls.append((a, m, e, d)))
            def fails_once() -> int:
    ...
    def test_retry_no_structlog_no_crash_expected(self) -> None:
        ...
        with patch("taipanstack.resilience.retry._HAS_STRUCTLOG", False), patch("taipanstack.resilience.retry._structlog_logger", None), patch("taipanstack.resilience.retry.time.sleep"):
            from taipanstack.resilience.retry import retry
            attempt_counter = {"n": 0}
...
class TestCircuitBreakerStructlogIntegration:
    ...
    def test_circuit_breaker_calls_structlog_on_state_change_without_callback_expected(self) -> None:
    ...
    def test_circuit_breaker_does_not_call_structlog_when_callback_provided_expected(self) -> None:
        """When on_state_change callback is set, structlog must NOT be called."""
        mock_structlog_logger = MagicMock()
        transitions: list[tuple[object, object]] = []
        with patch("taipanstack.resilience.circuit_breaker._HAS_STRUCTLOG", True), patch("taipanstack.resilience.circuit_breaker._structlog_logger", mock_structlog_logger):
            from taipanstack.resilience.circuit_breaker import CircuitBreaker
            breaker = CircuitBreaker(failure_threshold=1, name="callback_circuit", on_state_change=lambda o, n: transitions.append((o, n)))
    ...
    def test_circuit_breaker_no_structlog_no_crash_expected(self) -> None:
        ...
        with patch("taipanstack.resilience.circuit_breaker._HAS_STRUCTLOG", False), patch("taipanstack.resilience.circuit_breaker._structlog_logger", None):
            from taipanstack.resilience.circuit_breaker import CircuitBreaker
