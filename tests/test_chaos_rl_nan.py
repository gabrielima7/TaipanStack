...
def test_rate_limiter_chaos_time_corruption_expected(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_rate_limiter_chaos_time_corruption_has_tokens_expected(monkeypatch: pytest.MonkeyPatch) -> None:
    limiter = RateLimiter(max_calls=5, time_window=10.0)
    assert limiter.consume() is True
