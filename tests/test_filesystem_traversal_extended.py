...
class TestFilesystemTraversalExtended:
    ...
    def test_ensure_dir_traversal_patterns_expected(self, tmp_path: Path, pattern: str):
    ...
    @pytest.mark.parametrize("pattern", ["%2e%2e", "%252e%252e"])
    def test_safe_read_encoded_traversal_expected(self, tmp_path: Path, pattern: str):
    ...
    def test_safe_write_encoded_traversal_expected(self, tmp_path: Path):
        """Test that safe_write blocks encoded traversal patterns."""
        with pytest.raises(SecurityError):
            safe_write("%2e%2e/evil.txt", "content")

    def test_ensure_dir_with_explicit_base_dir_encoded_expected(self, tmp_path: Path):
        """Test ensure_dir with base_dir and encoded traversal."""
        with pytest.raises(SecurityError):
            ensure_dir("%2e%2e/evil", base_dir=tmp_path)

    def test_tilde_in_middle_not_blocked_expected(self, tmp_path: Path):
