...
def test_taipanstack_validators_url_recursive_unquote(payload, depth):
...
def test_taipanstack_guards_url_recursive_unquote(payload, depth):
    for _ in range(depth):
        payload = quote(payload)
    ...
    assert "URL contains invalid characters" in str(result.err())
