...
def test_chaos_retry_not_instance_exception_expected() -> None:
...
def test_chaos_retry_async_not_instance_exception_expected() -> None:
    @retry(max_attempts=2, on=(ValueError,))
...
def test_chaos_retry_single_exception_type_not_tuple_expected():
    call_count = 0

    @retry(max_attempts=2, on=ValueError)
    def test_func_expected():
        nonlocal call_count
        call_count += 1
        if call_count < 2:
    ...
    assert test_func_expected() == "success"
    assert call_count == 2


def test_chaos_retry_on_mutation_expected():
...
def test_chaos_retry_async_on_mutation_expected():
    corrupted_on = "NotAnException"
...
def test_chaos_retry_err_val_type_error_expected() -> None:
    corrupted_on = "NotAnException"
...
def test_chaos_retry_async_err_val_type_error_expected() -> None:
    corrupted_on = "NotAnException"
...
def test_chaos_retry_non_exception_class_in_tuple_expected() -> None:
