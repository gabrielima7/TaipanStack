...
@pytest.mark.asyncio
async def test_chaos_watchdogs_additional_chaos_watchdogs_base_watcher_stop_timeout_error_handles_timeouterror() -> (
    None
):
...
@pytest.mark.asyncio
async def test_chaos_watchdogs_additional_chaos_watchdogs_base_watcher_stop_oserror_like_oserror() -> (
    None
):
...
@pytest.mark.asyncio
async def test_chaos_watchdogs_additional_chaos_watchdogs_base_watcher_loop_timeout_error_from_stopeventwait() -> (
    None
):
...
@pytest.mark.asyncio
async def test_chaos_watchdogs_additional_chaos_watchdogs_base_watcher_start_stop_event_clear_expected() -> (
    None
):
    watcher = DummyWatcher(interval=0.1)
    await watcher.start()
