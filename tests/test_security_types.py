...
class TestSafeUrl:
    ...
    def test_valid_public_url_passes_expected(self) -> None:
...
class TestSafePath:
    ...
    def test_encoded_traversal_raises_expected(self) -> None:
        """URL-encoded path traversal raises ValidationError."""
        with pytest.raises(ValidationError):
            PathModel(path="%2e%2e/etc/shadow")

    def test_tilde_traversal_raises_expected(self) -> None:
...
class TestSafeCommand:
    ...
    def test_simple_command_passes_expected(self) -> None:
        """A benign command string passes validation."""
        m = CommandModel(command="ls")
        assert m.command == "ls"

    def test_command_with_semicolon_raises_expected(self) -> None:
...
class TestSafeHtml:
    ...
    @given(st.text())
    def test_property_all_strings_escaped_expected(self, text: str) -> None:
...
class TestSafeSqlIdentifier:
    ...
    def test_valid_identifier_passes_expected(self) -> None:
    ...
    def test_invalid_identifier_raises_expected(self) -> None:
    ...
    @given(st.from_regex("^[a-zA-Z_][a-zA-Z0-9_]*$", fullmatch=True))
    def test_property_valid_identifiers_pass_expected(self, text: str) -> None:
