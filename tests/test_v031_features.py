...
class TestGuardCommandInjectionTypeCheck:
    ...
    def test_v031_features_rejects_non_string_items_expected(self) -> None:
        with pytest.raises(TypeError, match="got int at index 2"):
            guard_command_injection(["git", "clone", 123])  # type: ignore[list-item]

    def test_v031_features_rejects_none_item_expected(self) -> None:
...
class TestGuardEnvVariableEdgeCases:
    ...
    def test_v031_features_rejects_non_string_name_expected(self) -> None:
        with pytest.raises(TypeError, match="Variable name must be str, got int"):
            guard_env_variable(123)  # type: ignore[arg-type]

    def test_v031_features_rejects_empty_name_expected(self) -> None:
        with pytest.raises(SecurityError, match="empty or whitespace"):
            guard_env_variable("")

    def test_v031_features_rejects_whitespace_only_name_expected(self) -> None:
        with pytest.raises(SecurityError, match="empty or whitespace"):
            guard_env_variable("   ")
...
class TestOnRetryCallback:
    ...
    def test_v031_features_on_retry_callback_invoked_expected(self) -> None:
...
class TestOnStateChangeCallback:
    ...
    def test_v031_features_callback_on_closed_to_open_expected(self) -> None:
    ...
    def test_v031_features_callback_on_full_lifecycle_expected(self) -> None:
